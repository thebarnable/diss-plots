...
def sg():
    # generate random data
    x = np.array([np.linspace(-1, 0, 50), np.linspace(0, 1, 50)]).flatten()
    y = np.array([np.zeros((50,)), np.ones((50,))]).flatten()
    ...
    ax[0].legend(handles=[l1, l2, l3], loc='upper right', bbox_to_anchor=(1.0, 0.4),fontsize=15, labels=["arctan", "rect", "H'"]) # bbox_to_anchor=(1.0, 1.25),
